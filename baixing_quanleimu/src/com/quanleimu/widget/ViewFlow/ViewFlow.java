 ...
 */
package com.quanleimu.widget.ViewFlow;
...
//		}
		...
		int item_height = 0, item_width = 0;
		final int count = getChildCount();
		for (int i = 0; i < count; i++) {
			getChildAt(i).measure(widthMeasureSpec, heightMeasureSpec);
			
			int height_temp = getChildAt(i).getMeasuredHeight();
			item_height = height_temp > item_height ? height_temp : item_height;
...
//			System.out.print("view flow item "+ i + "with height" + height);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			{
				performItemClick(this.getChildAt(mCurrentScreen), mCurrentScreen, 0);
			}
