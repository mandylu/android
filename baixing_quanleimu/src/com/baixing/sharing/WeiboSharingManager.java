...
public class WeiboSharingManager implements BaseSharingManager{
	class WeiboAccessTokenWrapper  extends Object implements Serializable{
		/**
		 * 
		 */
	...
	private void saveToken(WeiboAccessTokenWrapper token){
		Util.saveDataToLocate(mActivity, STRING_WEIBO_ACCESS_TOKEN, WeiboAccessTokenWrapper.class);
	}
	...
	public void share(Ad ad) {
		if(mToken != null && mToken.getExpires() != null && mToken.getExpires().length() > 0
				&& mToken.getToken() != null && mToken.getToken().length() > 0){
			doShare2Weibo(new Oauth2AccessToken(mToken.getToken(), mToken.getExpires()));
			return;
		}
		mAd = ad;
		// TODO Auto-generated method stub
		mWeibo = Weibo.getInstance(kWBBaixingAppKey, "http://www.baixing.com");
		mWeibo.authorize(mActivity, new WeiboAuthListener(){
			...
			public void onComplete(Bundle values) {				
	            ...
	            Log.d("weibosharingmanager", "token:  " + token);
	            Log.d("weibosharingmanager", "expires_in:  " + expires_in);
	            mToken = new WeiboAccessTokenWrapper(token, expires_in);
	            saveToken(mToken);
