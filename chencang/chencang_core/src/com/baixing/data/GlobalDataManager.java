...
//	public List<FirstStepCate> listFirst = new ArrayList<FirstStepCate>();
	...
	public Category findSpecifiedCategory(String firstLevelEnglishName){
		List<Category> allCates = getFirstLevelCategory();
		for(int i = 0; i < allCates.size(); ++ i){
			if(allCates.get(i).getEnglishName().equals(firstLevelEnglishName)){
				return allCates.get(i);
			}
		}
		return null;
	}
