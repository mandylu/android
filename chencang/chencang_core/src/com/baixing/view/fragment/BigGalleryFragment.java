...
//	private HashMap<String, byte[]> imageData;
	...
	static private String mediaPath = Environment.getExternalStorageDirectory().getPath();
	private Vector<String> unScannedFiles = new Vector<String>();
	
	private static String getPicStoragePath(){
		PackageManager packageManager = GlobalDataManager.getInstance().getApplicationContext().getPackageManager();
		ApplicationInfo ai = null;
		try {
			ai = packageManager.getApplicationInfo(GlobalDataManager.getInstance().getApplicationContext().getPackageName(), 0);
		...
		if(ai != null){
			CharSequence cs = packageManager.getApplicationLabel(ai);
			if(cs != null){
				path += "/" + cs.toString() + "/";
				return path;
			}
...
//	    	String filePath = SimpleImageLoader.getFileInDiskCache(vfCoupon.getSelectedView().getTag().toString());
	        ...
	        String mediaFileName = getPicStoragePath()+fileName.hashCode()+".png";
