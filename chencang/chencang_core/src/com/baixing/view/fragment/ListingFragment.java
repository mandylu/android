...
//		defaultExitAnim = 0;
	...
	public void onResume() {
		...
		String cat = GlobalDataManager.getInstance().getCategoryName();
		Log.e("ABC", "city and cat { " + city + "," + cat);
		if (TextUtils.isEmpty(city) || TextUtils.isEmpty(cat)) {
			this.requestCatAndCity();
		} else {
			this.initTitleAction(getView(), city, cat);
		}
		
		if (this.needRefresh) {
			
			this.showOrLoadFilter();
	...
	private void requestCatAndCity() {
		Intent intent = new Intent();
		intent.setClass(getActivity(), BaseActivity.cateSelectClass);
		this.startActivityForResult(intent, REQUEST_CODE_SETCITYORCAT);
	}
...
//		if (requestCode == REQUEST_CODE_SETCITYORCAT) {
			if (resultCode == Activity.RESULT_OK) {
				Log.d(TAG, "i was told set city and cat done.");
				resetCatAndCity();				
				needRefresh = true;
			} else if (resultCode == Activity.RESULT_CANCELED) {
				if (!TextUtils.isEmpty(getArguments().getString("categoryEnglishName")) 
						&& !TextUtils.isEmpty(GlobalDataManager.getInstance().getCityEnglishName())) {
					Log.d(TAG, "i was told user canceled setting. Do nothing here.");
				} else {
					getActivity().finish();
//					this.finishFragment();
				}
...
//				}				
	...
	private void initTitleAction(View root, String city, String cat) {
		Pair<String, String> category = CityAndCategorySelector.getInstance().parseCategory();
		boolean configable = category == null || CityAndCategorySelector.getInstance().isCitySelectable();
		View.OnClickListener titleActionListener = new View.OnClickListener() {
			
			@Override
			public void onClick(View v) {
				if(v.getId() == R.id.logo_root) {
					requestCatAndCity();
				} else if (v.getId() == R.id.filter) {
					handleRightAction();
				} else if (v.getId() == R.id.setting) {
					//TODO: go setting.
					pushFragment(new SettingFragment(), new Bundle());
				}
			}
		};
		
		if (configable) {
			root.findViewById(R.id.logo_root).setOnClickListener(titleActionListener);
			root.findViewById(R.id.title_action_indicator).setVisibility(View.VISIBLE);
		} else {
			root.findViewById(R.id.title_action_indicator).setVisibility(View.GONE);
		}
		
		TextView text = (TextView) root.findViewById(R.id.title_label_city_cat);
		text.setText(city + "-" + cat);
		
		root.findViewById(R.id.filter).setOnClickListener(titleActionListener);
		root.findViewById(R.id.setting).setOnClickListener(titleActionListener);
	}
...
//												Communication.E_DATA_POLICY.E_DATA_POLICY_ONLY_LOCAL));
	...
	private boolean isSettingDone() {
		return !TextUtils.isEmpty(GlobalDataManager.getInstance().getCityEnglishName()) && !TextUtils.isEmpty(GlobalDataManager.getInstance().getCategoryEnglishName());
	}
